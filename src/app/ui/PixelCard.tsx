...
const VARIANTS = {
  default: { activeColor: null as string | null, gap: 5, speed: 35, colors: '#f8fafc,#f1f5f9,#cbd5e1', noFocus: false },
  ...
  const initPixels = () => {
    ...
    for (let x = 0; x < width; x += Math.max(1, Math.trunc(finalGap))) {
      for (let y = 0; y < height; y += Math.max(1, Math.trunc(finalGap))) {
        const color = colorsArray[Math.floor(Math.random() * colorsArray.length)] || '#fff'
        const dx = x - width / 2
        const dy = y - height / 2
        const distance = Math.sqrt(dx * dx + dy * dy)
        const delay = reducedMotion ? 0 : distance
        pixels.push(new Pixel(canvas, ctx, x, y, color, getEffectiveSpeed(finalSpeed, reducedMotion), delay))
      }
    }
