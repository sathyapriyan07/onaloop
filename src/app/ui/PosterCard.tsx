...
export default function PosterCard({ to, title, posterUrl, logoUrl, aspect = 'poster' }: Props) {
  return (
    <Link
      to={to}
      className={clsx(
        'group relative block overflow-hidden rounded-2xl theme-card border',
    ...
    >
      {posterUrl ? (
        ...
        <div className="flex h-full w-full items-center justify-center" style={{ background: 'var(--bg-card)' }}>
          <span className="px-3 text-center text-sm font-medium" style={{ color: 'var(--text-muted)' }}>{title}</span>
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        {logoUrl ? (
          ...
          <div className="line-clamp-2 text-sm font-semibold tracking-tight text-white">{title}</div>
        )}
      </div>
    </Link>
  )
